...
test('Portable Viewer is self-contained, offline, and carries origin/capability metadata', () => {
  ...
  assert.ok(decodePortable(built.html).assets.asset_pixel)
  assert.ok(built.html.includes('src=""'))
  assert.ok(built.html.includes('sourceRevision'))
  assert.equal(decodePortable(built.html).origin.sourceDocumentId, document.documentId)
  assert.equal(decodePortable(built.html).origin.sourceRevision, canonicalRevision(document))
