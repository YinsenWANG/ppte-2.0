...
export const MCP_TOOL_INPUT_SCHEMAS: Record<AgentToolName | 'deliver_presentation', JsonSchema> = {
  ...
  get_recipe_controls: object({ recipeId: stringSchema, recipeVersion: stringSchema }, ['recipeId'], false),
  apply_design_theme: object({ theme: documentSchema, transactionId: stringSchema, requireConfirmation: booleanSchema }, ['theme'], false),
  apply_layout_recipe: object({ ...generatedProperties, parameters: { type: 'object', properties: { columns: { type: 'integer', enum: [1, 2, 3] } }, additionalProperties: false }, rebuildBinding: booleanSchema }, ['slideId'], false),
