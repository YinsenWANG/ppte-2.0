...
export interface PortableBuildOptions {
  ...
  exportOptions?: Record<string, string | number | boolean>
...
export interface PortablePayload {
  compatibilityDescriptor?: ReturnType<typeof profileDescriptor>
  fontPolicyVersion?: string
  exportOptions?: Record<string, string | number | boolean>
  buildManifest?: { applicationVersion: string; runtimeBuildId: string }
  buildVersion?: string
...
export function buildPortable(document: PpteDocument, options: PortableBuildOptions): PortableBuildResult {
  ...
  const payload: PortablePayload = { compatibilityDescriptor: profileDescriptor(inferCompatibilityProfile(document, options)), fontPolicyVersion: '1', exportOptions: options.exportOptions ?? {}, buildVersion: PPTE_APP_VERSION, document, origin, recentTransactions: options.recentTransactions ?? [], redoHistory: options.redoHistory ?? [], minimumCompatibilityProfile: inferCompatibilityProfile(document, options), assets, fonts, capabilityReport }
  let html = assembleHtml(document, payload, assetSources)
  payload.buildManifest = { applicationVersion: PPTE_APP_VERSION, runtimeBuildId: computeArtifactIdentity(payload, html).components.runtime }
...
export function auditPortableBundle(html: string): PortableAuditResult {
  try { return auditPortableContents(html) }
  catch (cause) { return { ok: false, issues: [issue('PORTABLE_INVALID', String(cause))] } }
}

function auditPortableContents(html: string): PortableAuditResult {
  ...
  // Resource declarations are checked independently of the self-reported identity.
  try {
    for (const asset of Object.values(payload.document.assets)) {
      const data = unbase64(payload.assets[asset.id] ?? '');
      if (data.length !== asset.byteLength || sha256Binary(data) !== normalizeHash(asset.hash)) issues.push(issue('ASSET_HASH_MISMATCH', `Embedded asset ${asset.id} differs from its declaration.`))
    }
    for (const font of Object.values(payload.document.fonts)) if (font.source === 'embedded') {
      const data = payload.fonts[font.id];
      if (!data || (font.hash && sha256Binary(unbase64(data)) !== normalizeHash(font.hash))) issues.push(issue('FONT_HASH_MISMATCH', `Embedded font ${font.id} differs from its declaration.`))
    }
    if (payload.compatibilityDescriptor && canonicalJsonString(payload.compatibilityDescriptor) !== canonicalJsonString(profileDescriptor(payload.minimumCompatibilityProfile!))) issues.push(issue('PORTABLE_CAPABILITY_MISMATCH', 'Compatibility descriptor differs from the supported contract.'))
    if (payload.buildManifest && (payload.buildManifest.applicationVersion !== payload.buildVersion || payload.buildManifest.runtimeBuildId !== computeArtifactIdentity(payload, html).components.runtime)) issues.push(issue('ARTIFACT_BUILD_MISMATCH', 'Build manifest differs from actual runtime bytes or application version.'))
  } catch (cause) { issues.push(issue('PORTABLE_INVALID', String(cause))) }
  if (payload.buildManifest && !payload.artifactIdentity) issues.push(issue('ARTIFACT_IDENTITY_MISSING', 'This build requires artifact identity.'))
...
export class PortableRuntime {
  ...
  commitPreparedVideo(slideId:string,elementId:string,prepared:PreparedVideo,migrateSource?:string):QuickFixResult {
    if(this.profile==='viewer'||!this.presentation.canMutate)return {ok:false,issues:[issue('PRESENTATION_READONLY','Video import requires edit mode.')]}
    try {
      const tx=planVideo(this.getDocument(),{revision:this.getRevision(),slideId,elementId,prepared,migrateSource,transactionId:`portable:video:${crypto.randomUUID()}`})
    ...
    }catch(cause){return {ok:false,issues:[issue('VIDEO_IMPORT_FAILED',String(cause))]}}
  }
