...
Task JSON (authoritative, from docs/usability-reset/TASKS.json):
{
 ...
 "title": "重做图片直接操作",
 "status": "pending",
 "acceptance": "拖动、四角等比缩放、直接裁切、取消与事务撤销；四类布局及保存重开",
 "evidence": []
}

Implement task U02 fully per its acceptance items; every item needs a real test or (for real-browser/human layers) an explicit pending entry with reason. Read docs/usability-reset/PLAN.md section U02 as the binding interaction contract. Update TASKS.json status/evidence honestly.