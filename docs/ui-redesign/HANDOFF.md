...
默认阅读、编辑上下文工具、纯净放映；统一插入文本框/形状/图片/表格；保存文案与真实权限一致。先看三种模式与交互，再按 UI01–UI06 接入已有模式、Commands、保存和资源管线。禁止用原型的固定坐标数据模型替换作者 HTML/CSS，禁止重引入必需 HTTP 服务、Office 或 MCP stdio。
