...
export function setMarks(doc: RichTextDocument, range: TextRange, patch: MarkPatch) {
  if (!validTextMarks(Object.fromEntries(Object.entries(patch).filter(([,value])=>value!==null)))) throw new Error('Invalid run marks')
  ...
  for (const p of content.paragraphs) {
    ...
    p.runs = p.runs.flatMap(r => {
      ...
      for (const [key,value] of Object.entries(patch)) {
        if (!['bold','italic','underline','strike','color','fontFamily','fontSize'].includes(key)) throw new Error('Unsupported mark')
...
export const clearMarks = (doc: RichTextDocument, range: TextRange) => setMarks(doc,range,{bold:null,italic:null,underline:null,strike:null,color:null,fontFamily:null,fontSize:null})
