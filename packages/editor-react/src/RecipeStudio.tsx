import { recipeControls, parameterizeRecipe } from '../../design-compiler/src/design-edits.js'
...
export function RecipeStudio({documentNode,slideId,assetSources,onApply,onClose}:{documentNode:PpteDocument;slideId:string;assetSources:Record<string,string>;onApply:(recipe:RecipeSpec,parameters:Record<string,unknown>)=>void;onClose:()=>void}):ReactElement {
  const [spec,setSpec]=useState(()=>builtInRecipeSpecs()[0]!)
  const [parameters,setParameters]=useState<Record<string,unknown>>({})
  const controls=recipeControls(spec)
  ...
  function select(r:RecipeSpec){setParameters({});setSpec(structuredClone(r));setZoneId(r.zones[0]?.id??'');setReport(undefined);setJson('')}
  ...
  return <section data-ppte-recipe-studio className="ppte-recipe-studio"><h3>布局工作室</h3><p>本浏览器已确认应用此版本 {accepted} 次（不代表其他用户或宿主的统计）。</p><p>拖动区域或调整归一化尺寸。约束会在编译时生效。应用布局前仍需预览确认。</p>
    ...
    <fieldset data-ppte-recipe-parameters><legend>配方参数（只调整布局，保留文字与局部格式）</legend>{controls.map(c=><label key={c.name}>{c.label}<select aria-label={c.label} value={String(parameters[c.name]??c.default)} onChange={e=>setParameters({...parameters,[c.name]:c.type==='integer'?Number(e.target.value):e.target.value})}>{c.options.map(v=><option key={v} value={String(v)}>{v}</option>)}</select></label>)}</fieldset>
    ...
    <button onClick={()=>{try{setReport(testRecipe(parameterizeRecipe(spec,parameters).recipe,documentNode,slideId,assetSources));setMessage('测试完成；失败案例和诊断均保留在报告中')}catch(e){setMessage(String(e))}}}>批量测试当前页与边界样本</button>
    ...
    <p role="status">{message}</p><button onClick={()=>{try{new RecipeRegistry([spec]);parameterizeRecipe(spec,parameters);onApply(spec,parameters)}catch(e){setMessage(String(e))}}}>预览应用到当前页</button><button onClick={onClose}>关闭工作室</button>
  </section>
}
