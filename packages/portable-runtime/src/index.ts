...
export function buildPortable(document: PpteDocument, options: PortableBuildOptions): PortableBuildResult {
  ...
  const budgetBytes = options.profile === 'viewer' ? 1_200_000 : options.profile === 'quick-fix' ? 2_000_000 : 3_000_000
...
function portableScript(): string {
  ...
  const safeFilename=value=>String(value||'presentation').replace(/[^A-Za-z0-9._-]+/g,'_').replace(/^[.]+|[.]+$/g,'')||'presentation';
  ...
  const hydrateFonts=()=>{let style=document.getElementById('ppte-portable-fonts');if(style)style.remove();const rules=[];for(const font of Object.values(documentNode.fonts||{})){const encoded=payload.fonts&&payload.fonts[font.id];if(encoded)rules.push('@font-face{font-family:"'+String(font.family).replace(/[^A-Za-z0-9 ,._-]/g,'')+'";font-style:'+font.style+';font-weight:'+font.weight+';src:url(data:font/woff2;base64,'+encoded+') format("woff2");font-display:block}')};if(rules.length){style=document.createElement('style');style.id='ppte-portable-fonts';style.textContent=rules.join('');document.head.appendChild(style)}};
