...
</style></head><body><div id="ppte-shell" class="ppte-${payload.origin.profile}" data-ppte-profile="${payload.origin.profile}" data-ppte-deliverable="${editable ? 'true' : 'false'}" data-ppte-deliverable-role="${editable ? 'editable-browser-copy' : 'read-only-preview'}"><div class="ppte-toolbar"><strong class="ppte-brand">PPTe<span>演示文稿</span></strong><div class="ppte-tool-group"><button type="button" data-ppte-action="previous" aria-label="上一页">←</button><button type="button" data-ppte-action="next" aria-label="下一页">→</button></div><span class="ppte-status" data-ppte-status></span>${editingControls}<button type="button" class="ppte-primary" data-ppte-action="fullscreen">开始演示（全屏）</button></div><button type="button" class="ppte-exit" data-ppte-action="exit-present" aria-label="退出放映，返回编辑">退出放映 · Esc</button><main class="ppte-stage" data-ppte-stage><div class="ppte-canvas" data-ppte-canvas style="width:${document.canvas.width}px;height:${document.canvas.height}px">${rendered}</div></main><div class="ppte-notes" data-ppte-notes aria-live="polite"></div></div><script id="ppte-portable-payload" type="application/json">${payloadJson}</script><script id="ppte-runtime">${portableScript()}</script></body></html>`
}
