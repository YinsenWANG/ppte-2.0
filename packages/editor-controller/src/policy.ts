import type { Transaction, ValidationIssue } from '../../schema/src/index.js'
export type EditorProfile = 'host' | 'viewer' | 'quick-fix' | 'light-edit' | 'full-portable'
const quick = new Set(['text.replaceContent', 'asset.upsert', 'image.replaceAsset', 'fact.upsert', 'fact.syncReferences'])
const light = new Set([...quick, 'image.setCrop', 'chart.replaceData', 'element.move', 'element.resize', 'element.rotate'])
export function commandPolicy(profile: EditorProfile, mutable: boolean, transaction?: Transaction): ValidationIssue[] {
