...
export function envelope(content: string, metadata: Metadata) {
  ...
  return `<!doctype html>\n<html lang="zh-CN"><head><meta charset="utf-8"><meta http-equiv="Content-Security-Policy" content="${policy}"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeText(title ? textOf(title) : 'PPTe HTML')}</title><style>html,body{margin:0;width:100%;height:100%;background:#111}#ppte-frame{display:block;width:100%;height:100%;border:0}</style></head><body><template id="ppte-content">${escapeText(packed?.content ?? content)}</template>${media}<script id="ppte-metadata" type="application/json">${JSON.stringify(metadata).replace(/</g, '\\u003c')}</script><iframe id="ppte-frame" title="演示稿" sandbox="allow-same-origin" referrerpolicy="no-referrer"></iframe><script id="ppte-runtime">${script}</script></body></html>`;
}
