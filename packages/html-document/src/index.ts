...
export interface Metadata { documentId: string; formatVersion: 1; saveRevision: number }
...
export function envelope(content: string, metadata: Metadata) {
  ...
  return `<!doctype html>\n<html lang="zh-CN"><head><meta charset="utf-8"><meta http-equiv="Content-Security-Policy" content="${policy}"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeText(title ? textOf(title) : 'PPTe HTML')}</title><style>html,body{margin:0;width:100%;height:100%;background:#111}#ppte-frame{display:block;width:100%;height:100%;border:0}</style></head><body><template id="ppte-content">${escapeText(content)}</template><script id="ppte-metadata" type="application/json">${JSON.stringify(metadata).replace(/</g, '\\u003c')}</script><iframe id="ppte-frame" title="演示稿" sandbox="allow-same-origin" referrerpolicy="no-referrer"></iframe><script id="ppte-runtime">${script}</script></body></html>`;
}
export async function enhanceHTML(input: string, options: ResourceOptions) {
  ...
  return { html: envelope(cleaned.html, metadata), pages: cleaned.pages, issues: cleaned.issues, mediaBytes: embedded.mediaBytes, metadata };
}
/** Save service boundary: rebuild trusted envelope, never save a caller-supplied runtime. */
export function serializeContent(content: string, metadata: Metadata) {
  const cleaned = cleanContent(content);
  if (cleaned.issues.length) throw Error('UNSAFE_CONTENT: ' + JSON.stringify(cleaned.issues));
  return envelope(cleaned.html, { ...metadata, saveRevision: metadata.saveRevision + 1 });
}
