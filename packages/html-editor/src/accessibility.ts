...
export function disclosure(details:HTMLDetailsElement, menu=false) {
 ...
 const place=()=>{if(!details.open)return;body.style.position='fixed';body.style.bottom='auto';const r=trigger.getBoundingClientRect();body.style.left=Math.max(8,Math.min(r.left,innerWidth-body.offsetWidth-8))+'px';body.style.right='auto';body.style.top=Math.max(8,Math.min(r.bottom+4,innerHeight-body.offsetHeight-8))+'px';};
