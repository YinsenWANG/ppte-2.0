...
function capabilityForElement(document: PpteDocument, slideId: string, element: Element, target: CapabilityTarget): CapabilityItem {
  ...
  if (element.type === 'text') {
    const glyph = inspectGlyphCoverage(document, element, undefined, { strict: target === 'portable-quick-fix' })
    const glyphIssues = checkGlyphCoverage(document, element, undefined, { strict: target === 'portable-quick-fix' })
    if (glyphIssues.length) {
      status = target === 'portable-viewer' ? 'font-replacement' : 'blocked'
