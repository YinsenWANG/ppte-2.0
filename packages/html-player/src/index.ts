...
export function installPlayer(frame: HTMLIFrameElement, hooks: {
    ...
    let running = false, index = 0, step = 0, black = false;
...
#ppte-player-controls{position:fixed;z-index:500;bottom:16px;left:50%;transform:translateX(-50%);background:#20242c;color:white;border-radius:12px;padding:8px;display:flex;gap:8px;opacity:0;pointer-events:none}
#ppte-player-controls[data-visible]{opacity:1;pointer-events:auto}#ppte-player button{font:14px system-ui;padding:8px;border-radius:6px;border:0;color:#20242c;background:#fff}#ppte-player button:focus-visible{outline:3px solid #7194ff;outline-offset:3px}
#ppte-black{display:none;position:fixed;inset:0;background:#000;z-index:9999}#ppte-player[data-black] #ppte-black{display:block}#ppte-player[data-black] #ppte-player-controls{display:none}
</style><div id="ppte-player-controls" role="toolbar" aria-label="放映控制"></div><div id="ppte-black" aria-label="黑屏"></div>`;
    document.body.append(shell);
    const controls = shell.querySelector<HTMLElement>('#ppte-player-controls')!;
    function wake() {
        if (!running || black) return;
        controls.dataset.visible = ''; clearTimeout(timer);
        timer = setTimeout(() => { controls.removeAttribute('data-visible'); if (controls.contains(document.activeElement)) frame.focus(); }, 1800);
    }
    ...
    function blackout(value = !black) { black = value; shell.toggleAttribute('data-black', black); if (black) { pause(); controls.removeAttribute('data-visible'); frame.focus(); } }
    function exit() {
        if (!running) return;
        pause(); running = false; black = false; shell.hidden = true; shell.removeAttribute('data-black'); css?.remove(); css = undefined; clearTimeout(timer);
    ...
    function keys(e: KeyboardEvent) {
        ...
        if (['Escape','ArrowRight','ArrowLeft','ArrowDown','ArrowUp','PageDown','PageUp',' ','b','B','.','p','P','Tab'].includes(e.key)) {
            ...
            else if (e.key === 'Tab') { wake(); if (black) e.preventDefault(); }
    ...
    function presenter() {
        ...
        if (!speaker) { wake(); controls.querySelector('[aria-live]')!.textContent = '演讲者窗口被阻止；可继续放映，按 Esc 退出'; return; }
        const d = speaker.document;
    ...
    for (const [label,action] of [['上一页',()=>next(true)],['下一步',()=>next()],['黑屏 / 恢复',()=>blackout()],['演讲者视图',presenter],['退出放映',exit]] as const) { const b=document.createElement('button');b.textContent=label;b.onclick=action;controls.append(b); }
    ...
    controls.addEventListener('pointermove',wake); controls.addEventListener('focusin',wake);
    function attach() {
        frame.contentDocument!.addEventListener('keydown',keys,true);
        frame.contentDocument!.addEventListener('pointermove',wake);
        frame.contentDocument!.addEventListener('click',e=>{ if (running && !(e.target as Element).closest('video,audio,a,area')) next(); });
    }
    attach(); frame.addEventListener('load',()=>{if(running)exit();attach();});
    document.addEventListener('keydown',keys,true);
    document.addEventListener('fullscreenchange',()=>{if(running && !document.fullscreenElement)exit();});
    ...
    return { get running(){return running;}, get index(){return index;}, get step(){return step;}, get black(){return black;}, next, exit, presenter,
        start() {
            if (running) return;
            ...
            hooks.suspend(); running=true; shell.hidden=false;controls.removeAttribute('data-visible');
            ...
            try { void document.documentElement.requestFullscreen?.().catch(()=>{}); } catch { /* Fullscreen is optional. */ }
        }
    };
}
