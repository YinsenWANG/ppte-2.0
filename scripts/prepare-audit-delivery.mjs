...
// A new round uses a new staging directory; enhancement never overwrites a work.
const evidence=resolve(process.argv[2]??'docs/audits/2026-09-08-main-52e3bf0/evidence/DLV');
...
try{
 ...
 const help=JSON.parse(run(process.execPath,[cli,'--help']));
 assert.match(help.pdf,/unfinished/);assert.match(help.pdf,/disabled/);assert.doesNotMatch(help.pdf,/More → Export PDF/);
 run(process.execPath,[cli,'skill-install','--out',skill]);
