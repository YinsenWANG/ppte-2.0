...
async function property(p:Page,label:string,value:string){if(await legacyImageProperty(p,label,value))return;const input=p.getByLabel(label,{exact:true});if(!await input.isVisible())await p.locator('#ppte-properties summary').filter({hasText:'位置与布局'}).click();await input.fill(value);await input.press('Tab');}
...
test('UI06 A1/A2: product offline four-object history → download → full shutdown → copy → preview/restore → download → fresh read/show/PDF', {skip:'F01 scope retirement: docs/focused-product/evidence/F01/TEST-MIGRATION.json; not a pass'}, async()=>{
 ...
 try{
  ...
  const chooser=p.waitForEvent('filechooser');await legacyImageInsertion(p);await p.getByRole('button',{name:'插入图片',exact:true}).click();await(await chooser).setFiles({name:'two-tones.png',mimeType:'image/png',buffer:Buffer.from(png.split(',')[1],'base64')});await frame(p).locator('img').waitFor();await property(p,'宽度','240px');await property(p,'高度','120px');await legacyImageCrop(p,'填充裁切');await property(p,'水平焦点（0–100%）','25');
  ...
  await p.getByRole('button',{name:'关闭版本历史',exact:true}).click();await p.getByRole('button',{name:'编辑',exact:true}).click();await p.getByRole('button',{name:'下一页',exact:true}).click();await frame(p).locator('[data-ppte-kind=text]').fill('恢复前 · 未保存的想法');await history(p);start=performance.now();await target.getByText('管理版本',{exact:true}).click();await target.getByRole('button',{name:'恢复到此版本',exact:true}).click();assert.match(await p.locator('#ppte-version-form').innerText(),/当前内容会先保留/);await p.getByRole('button',{name:'确认恢复',exact:true}).click();await p.locator('#ppte-version-form').waitFor({state:'detached'});await p.waitForFunction(()=>!(window as any).PPTeHTML.content().includes('恢复前 · 未保存的想法'));timings.restoreMs=performance.now()-start;assert.equal(await p.evaluate(()=>(window as any).PPTeHTML.content()),first);
