...
test('UI07 A1–A6: product controls, actual offline download, copied file in fresh process, preview/restore and media timings',async()=>{
 ...
 try{
  ...
  await page.getByText('更多',{exact:true}).click();await page.getByRole('button',{name:'版本历史',exact:true}).click();
  ...
  await page.getByRole('button',{name:'版本历史',exact:true}).click();page.once('dialog',d=>d.accept('第二稿'));await page.getByRole('button',{name:'保存命名版本',exact:true}).click();await page.getByRole('button',{name:'关闭版本历史',exact:true}).click();
  ...
  await page.getByText('更多',{exact:true}).click();await page.getByRole('button',{name:'版本历史',exact:true}).click();assert.equal(await page.getByRole('button',{name:'恢复到此版本',exact:true}).count(),0);
  ...
  await page.getByRole('button',{name:'版本历史',exact:true}).click();page.once('dialog',d=>{assert.match(d.message(),/当前内容会先保留/);return d.accept();});t=start();await target.getByRole('button',{name:'恢复到此版本',exact:true}).click();await page.waitForFunction(()=>(window as any).PPTeHTML.content().includes('第一稿'));timings.restoreMs=start()-t;
...
test('UI07 A3/A5/A6: real disk bridge writes media history; quota never blocks current save; no-history export removes private resources',async()=>{
 ...
 try{
  ...
  await page.getByRole('button',{name:'编辑',exact:true}).click();await page.frameLocator('#ppte-frame').locator('h1').fill('命名大图');await page.getByText('更多',{exact:true}).click();await page.getByRole('button',{name:'版本历史',exact:true}).click();page.once('dialog',d=>d.accept('私有旧图'));await page.getByRole('button',{name:'保存命名版本',exact:true}).click();await page.getByRole('button',{name:'关闭版本历史',exact:true}).click();await page.getByText('更多',{exact:true}).click();
