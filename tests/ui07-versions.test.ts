...
test('UI07 A1–A6: product controls, actual offline download, copied file in fresh process, preview/restore and media timings', {skip:'F01 scope retirement: docs/focused-product/evidence/F01/TEST-MIGRATION.json; not a pass'}, async()=>{
 ...
 try{
  ...
  await page.getByText('更多',{exact:true}).click();await page.getByRole('menuitem',{name:'版本历史',exact:true}).click();t=start();await target.getByText('管理版本',{exact:true}).click();await target.getByRole('button',{name:'恢复到此版本',exact:true}).click();assert.match(await page.locator('#ppte-version-form').innerText(),/当前内容会先保留/);await page.getByRole('button',{name:'确认恢复',exact:true}).click();await page.locator('#ppte-version-form').waitFor({state:'detached'});await page.waitForFunction(()=>(window as any).PPTeHTML.content().includes('第一稿'));timings.restoreMs=start()-t;
...
test('UI07 A3/A5/A6: real disk bridge writes media history; quota never blocks current save; no-history export removes private resources', {skip:'F01 scope retirement: docs/focused-product/evidence/F01/TEST-MIGRATION.json; not a pass'}, async()=>{
 ...
 try{
  ...
  await page.getByRole('button',{name:'编辑',exact:true}).click();await page.frameLocator('#ppte-frame').locator('h1').fill('命名大图');await page.getByText('更多',{exact:true}).click();await page.getByRole('menuitem',{name:'版本历史',exact:true}).click();await page.getByRole('button',{name:'保存命名版本',exact:true}).click();await page.getByLabel('版本名称',{exact:true}).fill('私有旧图');await page.getByRole('button',{name:'保存名称',exact:true}).click();await page.getByRole('button',{name:'关闭版本历史',exact:true}).click();
