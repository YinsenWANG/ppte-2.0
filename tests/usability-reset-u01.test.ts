...
// Historical signed-off representative fixture remains byte-identical. Full deck has separate coverage.
const file=join(evidence,'verification/representative-snapshot.html');
...
async function open(document=file){
...
test('U01 A3/full-deck: explicit decision gate preserves rejected human status and blocks expansion',async()=>{
 ...
 if(current.status!==task.status){
  assert.equal(current.status,'full-deck-awaiting-user-acceptance');
  assert.equal(current.fullDeck.status,'complete-awaiting-user-acceptance');
  assert.equal(current.verification.code,'passed');
  assert.equal(current.verification.human,'pending');
  assert.equal(current.humanRevalidation.status,'pending');
  assert.equal(current.fullDeck.pages,9);
...
test('U01 FULL A1/A2: nine native pages in one offline runtime, text/image bounds and clean presentation',async()=>{
...
test('U01 FULL A1: actual text editing, undo/redo, explicit download and new browser process retain the full source',async()=>{
 const file=join(evidence,'Cherry-Studio-开源之路.ppte.html');
