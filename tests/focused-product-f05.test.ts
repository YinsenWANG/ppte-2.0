...
test('F05 A1/A2/A3: same Cherry draft at 1440/1024/390, unclipped centered page, inspector, save status and pure presentation',async()=>{
 ...
 for(const width of [1440,1024,390]){
  ...
  await p.locator('#ppte-save-panel summary').click();await p.waitForFunction(()=>document.querySelector('#ppte-save-panel summary')?.getAttribute('aria-expanded')==='true');await fits(p,'#ppte-save-panel > div');assert.match(await p.locator('.save-detail').innerText(),/尚未写入/);assert.equal(await btn(p,'下载更新后的文件').count(),0);await p.evaluate(()=>{const w=window as any;w.__f05picker=w.showOpenFilePicker;w.showOpenFilePicker=undefined;w.PPTeSave.set(w.PPTeSave.state,w.PPTeSave.detail);});assert.equal(await btn(p,'下载更新后的文件').innerText(),'下载更新后的文件');await p.evaluate(()=>{const w=window as any;w.showOpenFilePicker=w.__f05picker;w.PPTeSave.set(w.PPTeSave.state,w.PPTeSave.detail);});
