...
// DPR 3 keeps this 640px raster at native decode resolution in both modes,
// avoiding Chromium decode downsampling changes in strict pixel comparisons.
const cases=[
...
async function setup(name:string,fit:string,position:string){
 await mkdir(out,{recursive:true});const browser=await chromium.launch({channel:'chrome',headless:true});const p=await browser.newPage({offline:true,deviceScaleFactor:3,viewport:{width:1440,height:1000}});p.setDefaultTimeout(10000);
...
for(const fit of ['cover','contain'])for(const c of cases)test(`R03 ${fit} ${c.name}: browser preview, no-op bytes/history, actual move/download/fresh reopen`,async()=>{
 ...
 try{
  const original=await content(p);await b(p,'阅读').click();const before=await image(p).screenshot({caret:'initial'});await b(p,'编辑').click();await image(p).click();await writeFile(join(out,name+'-before.png'),before);
  ...
  await b(p,'阅读').click();assert.deepEqual(await image(p).screenshot({caret:'initial'}),before,'no-op rendered image pixels unchanged');await b(p,'编辑').click();await image(p).click();
  ...
  await browser.close();fresh=await chromium.launch({channel:'chrome',headless:true});const q=await fresh.newPage({offline:true,deviceScaleFactor:3,viewport:{width:1440,height:1000}});
  for(const [file,expected] of [[unchanged,original],[saved,moved]]){
   await q.goto(pathToFileURL(file).href);await q.waitForFunction(()=>!!(window as any).PPTeSave);assert.equal(await content(q),expected);if(file===unchanged)assert.deepEqual(await image(q).screenshot({caret:'initial'}),before,'fresh process no-op pixel equality');await b(q,'编辑').click();await image(q).click();
