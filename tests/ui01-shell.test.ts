...
async function setup(name:string) {
 ...
 return {browser,page,file,async close(){assert.deepEqual(errors,[]);assert.deepEqual(network,[]);await writeFile(join(out,name+'.json'),JSON.stringify({status:'passed',browser:browser.version(),headless:true,offline:true,errors,network,time:new Date().toISOString()},null,2));await browser.close();}};
}
